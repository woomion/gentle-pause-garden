...
const PausedItemCard = ({ item, onClick, onDelete, onDecideNow, onEdit, currentUserId }: PausedItemCardProps) => {
  ...
  const handleEditSave = useCallback((updates: Partial<PausedItem | LocalPausedItem>) => {
    if (onEdit) {
      onEdit(item, updates);
    }
