...
const Auth = () => {
  ...
  const { signUp, signIn, signInWithGoogle, signInWithMagicLink, resetPassword, updatePassword } = useAuth();
  const { toast } = useToast();
  const navigate = useNavigate();

  // Check if user came from an invitation link or password reset
  useEffect(() => {
    ...
    if (isForgotPassword) {
      ...
      try {
        ...
        if (error) {
          ...
          toast({
            title: "Check your email",
            description: "We've sent you a password reset link. Check your email and follow the instructions.",
          });
          setIsForgotPassword(false);
        }
      } catch (error) {
      ...
      <div className="max-w-md w-full space-y-8">
        ...
        <div className="bg-white/60 dark:bg-white/10 rounded-2xl p-8 border border-lavender/30 dark:border-gray-600 relative">
            ...
            {!isForgotPassword && !isMagicLink && (
              <div>
                ...
                <Input
                  ...
                  required
                />
                {(isSignUp || isResetPassword) && <PasswordValidation password={password} />}
              </div>
            )}
            ...
            <div className="mt-6">
              ...
              <div className="mt-4 space-y-3">
                <Button
                  ...
                  disabled={loading}
                  className="w-full bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-900 dark:text-gray-100 font-medium py-3 px-6 rounded-2xl transition-all duration-200 transform hover:scale-[1.02] active:scale-[0.98] shadow-md flex items-center justify-center gap-3"
                >
                ...
                <Button
                  ...
                  disabled={loading}
                  className="w-full bg-transparent border-2 border-taupe hover:bg-taupe/10 text-black dark:text-[#F9F5EB] font-medium py-3 px-6 rounded-2xl transition-all duration-200 transform hover:scale-[1.02] active:scale-[0.98] shadow-md"
                >
                  Sign in with magic link
                </Button>
              </div>
          ...
          <div className="mt-6 text-center space-y-3">
            ...
            {!isSignUp && !isForgotPassword && !isResetPassword && !isMagicLink && (
              <div>
                <button
                  type="button"
